...
# Removed this temporarily as was successful with 30FPS more often than 60FPS
assert int(frameRate) >= 30, "Video must be minimum of 30FPS"

showParse = False

# count the number of red lines found in a portion of an image
# input image must be 3 pixels wide and in BGR format
def countLines(im):
  assert im.shape[1] == 3
  ...
  for y in range(im.shape[0]):
    ...
    if breakS >= 2:
      ...
      # increment line count if there were enough pixels in a row
      if streak > 15:
        lineC += 1
    ...
    # Used to check for deaths because they are a solid color and not an outlined box
...
while (cap.isOpened()):
  ...
  # check current number of lines vs previous frames number of lines
  if nlines < plines:
  ...
  # cv2.imshow('Area or Interest', frame)
